...
import (
	dpfm_api_caller "data-platform-api-business-partner-reads-general-rmq-kube/DPFM_API_Caller"
	...
	"data-platform-api-business-partner-reads-general-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, errs := caller.AsyncBusinessPartnerGeneralReads(accepter, &input, &output, l)
	if len(errs) != 0 {
...
func getAccepter(input *dpfm_api_input_reader.SDC) []string {
	...
	if accepter[0] == "All" {
		accepter = []string{
			"General", "GeneralPDF", "Role", "FinInst", "Relationship", "Accounting",
		}
	}
	return accepter
}
